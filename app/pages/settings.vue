...
import { ChevronLeft, ChevronDown, ChevronRight, Check, Smartphone, Moon, Sun, Monitor, Clock, Lock, Trash2, Zap, ShoppingCart, KeyRound } from 'lucide-vue-next'
import { settings, saveSettings, CURRENCIES, ACCENT_COLORS, orbLog, bills, groceryLists, activeListId } from '../composables/useStore'
...
function executeDanger() {
  ...
  else if (dangerTarget.value === 'bills') { try { localStorage.removeItem('orb_bills_v1') } catch {}; bills.value = []; orbLog('Bills cleared'); dangerTarget.value = null }
  else if (dangerTarget.value === 'grocery') { try { localStorage.removeItem('orb_grocery_lists_v1') } catch {}; groceryLists.value = []; activeListId.value = null; orbLog('Grocery cleared'); dangerTarget.value = null }
}
</script>
